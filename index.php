...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if ($errors['fio']) {
        ...
        $messages[] = '<div class="error1">Заполните имя</div>';
    }
    if ($errors['email']) {
        setcookie('email_error', '', 100000);
        $messages[] = '<div class="error1">Заполните почту</div>';
    }
    if ($errors['sex']) {
        setcookie('sex_error', '', 100000);
        $messages[] = '<div class="error1">Выберите пол</div>';
    }
    if ($errors['bio']) {
        setcookie('bio_error', '', 100000);
        $messages[] = '<div class="error1">Введите биографию</div>';
    }
    if ($errors['abil']) {
        setcookie('abil_error', '', 100000);
        $messages[] = '<div class="error1">Выберите сверхспособность</div>';
    }
    if ($errors['year']) {
        setcookie('year_error', '', 100000);
        $messages[] = '<div class="error1">Корректно введите дату рождения</div>';
    }
    if ($errors['limb']) {
        setcookie('limb_error', '', 100000);
        $messages[] = '<div class="error1">Выберите количество конечностей</div>';
    }
